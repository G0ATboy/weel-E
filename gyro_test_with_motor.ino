...
int c = 0;
int ThrottlePin = 11;    // throttle connected to digital pin 11
void setup() {
  Serial.begin(9600); //should be 19200
...
void loop() {
  ...
  roll = roll+40; //motor doesn't respond until analog output is at least 40/256
  if (roll <= 40) {
    roll = 0; }
...
//  Serial.println(yaw);
  ...
  //analog output 0-2.5V for controlled speed of motor
  //designate which pin for throttle earlier in script
  //truncate output of MPU to only include positive values (make negative values =0)
  //0-360degrees = 0-2.5V
  analogWrite(ThrottlePin, roll);
